...
pub struct VM {
    pub ctx: ModuleExecutionContext,
    pub state: VMState,
...
impl Default for VM {
    fn default() -> Self {
        let call_depth = if cfg!(debug_assertions) { 256 } else { 1024 };
        VM::new(ModuleExecutionContext::default(), call_depth)
    }
}
...
impl VM {
    ...
    pub fn execute_module(&mut self, module: &Module, start: usize) -> RuntimeResult {
        self.reset();
    ...
    pub fn execute_code(
        ...
        loop {
            match &code[ip] {
                ...
                LoadConst(index) => {
                    let obj = code.get_const(*index).unwrap();
                    self.push(ValueStackKind::Constant(obj.clone(), *index));
                }
                // Modules
                LoadModule(name) => {
                ...
                JumpIf(addr, forward, scope_exit_count) => {
                    ...
                    if let Some(should_jump) = obj.bool_val() {
                        ...
                        // XXX: Should not be a hard error?
                        return Err(RuntimeErr::type_err(format!(
                            "Cannot evaluate as bool: {obj}"
                        )));
                    }
                }
    ...
    fn halt(&mut self, exit_code: u8) -> RuntimeResult {
        self.reset();
        self.state = VMState::Halted(exit_code);
        Err(RuntimeErr::exit(exit_code))
    }

    pub fn halt_top(&mut self) -> RuntimeResult {
        let obj = self.pop_obj()?;
        let obj = obj.read().unwrap();
    ...
    fn handle_compare_op(&mut self, op: &CompareOperator) -> RuntimeResult {
        ...
        let result = match op {
            ...
            LessThan => a.less_than(b).unwrap_or(false),
            LessThanOrEqual => a.less_than(b).unwrap_or(false) || a.is_equal(b),
    ...
    pub fn call_intrinsic_func(
        ...
        let return_val = (func.func())(self.find_this(), args);
