...
fn new() {
    ...
    check_token(tokens.last(), Token::EndOfInput, 1, 1);
}
...
fn scan_string_with_embedded_quote() {
    ...
    check_token(tokens.last(), Token::EndOfInput, 1, 8);
}
...
fn scan_string_with_newline() {
    ...
    check_token(tokens.last(), Token::EndOfInput, 2, 2);
}
...
fn scan_string_with_many_newlines() {
    ...
    check_token(tokens.get(0), Token::Whitespace, 1, 1);
    check_token(tokens.get(1), Token::String(expected_string), 1, 3);
    check_token(tokens.last(), Token::EndOfInput, 7, 4);
}
            ...
            Token::UnterminatedString(string) => {
                ...
                match scanner.scan(new_input.as_str()) {
                    Ok(tokens) => {
                        assert_eq!(tokens.len(), 2);
                        check_token(tokens.get(0), Token::String("abc".to_string()), 1, 1);
                        check_token(tokens.last(), Token::EndOfInput, 1, 6);
                    }
                    _ => assert!(false),
                }
            }
...
fn scan_unknown() {
    ...
    check_token(tokens.last(), Token::EndOfInput, 1, 3);
}
