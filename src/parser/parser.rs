...
impl<I: Iterator<Item = ScanTokenResult>> Parser<I> {
    ...
    fn statement(&mut self) -> StatementResult {
        ...
        use Token::{
            Break, Continue, EndOfStatement, Import, Jump, Label, Return, Throw,
        };
    ...
    /// Handle `throw`.
    fn throw(&mut self, start: Location) -> StatementResult {
        let expr = self.expr(0)?;
        let end = expr.end;
        Ok(ast::Statement::new_throw(expr, start, end))
    }
